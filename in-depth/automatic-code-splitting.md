...
关于如何解决此问题，已经有一些讨论。一种方式是在原始顺序会被破坏时，把模块移入额外的公共代码块，但这会使输出过于碎片化。Rolldown 改为提供 [`strictExecutionOrder`](https://rolldown.rs/reference/OutputOptions.strictExecutionOrder)：它会包装 ESM 模块主体，使其在保持 ESM 输出的同时按源代码顺序运行。当被包装的动态入口最终与其他代码共享其实现代码块时，改写后的 `import()` 会直接触发该实现。严格模式只会在仍然需要实际文件的位置生成一个小型入口门面，例如另一个代码块静态加载该入口的代码块时，或对于插件生成的代码块。因此，输出代码块的形态仍可能改变。默认情况下，严格模式会包装所有符合条件的模块；实验性的 `onDemandWrapping` 模式则根据预测的代码块执行风险，推导出一组保守的模块子集。
